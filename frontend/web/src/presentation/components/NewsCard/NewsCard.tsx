...
export const NewsCard: React.FC = () => {
    ...
    return (
        <div className="news">
            { isNewsLoaded ? <NewsBlock news={data[position]} /> : <PuffLoader isLoading={loading} /> }
            <div className="news__toggle-bar">
                <button onClick={() => togglePosition('prev')} className="news__toggle-btn"><FontAwesomeIcon icon={faChevronLeft} /></button>
                <button onClick={() => togglePosition('next')} className="news__toggle-btn"><FontAwesomeIcon icon={faChevronRight} /></button>
            </div>
        </div>
    )
}