import { Configuration, Logger, Thread, TinkoffMarket } from "../../infra";
...
@injectable()
export class AutomatedTradingScheduler extends SchedulerBase {
    protected cronTime = "* * * * * *";
    protected onStart = false;

	private logger: Logger = ioc.resolve(Logger);
    ...
    async job(): Promise<void> {
		...
		const automatedUsers = await userService.find({ strategies: [UserStrategy.Automated] });
		for (const user of automatedUsers) {
			...
			this.logger.info(JSON.stringify(forSell));
		}
    }
}
